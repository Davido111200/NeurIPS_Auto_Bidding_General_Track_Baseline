...
import numpy as np
import uuid
from bidding_train_env.baseline.dt.sequence_dataset import SequenceDataset, DataLoader, discounted_cumsum
# from sequence_dataset import SequenceDataset, DataLoader, discounted_cumsum
...
class TrainConfig:
    # wandb params
    project: str = "NeurIPS_Auto_Bidding"
    group: str = "DT-D4RL"
    name: str = "DT"
    # model params
    embedding_dim: int = 128
...
def train(config: TrainConfig):
    ...
    dataset = SequenceDataset(dataset=dataset, seq_len=config.seq_len, reward_scale=config.reward_scale)
    ...
    for step in trange(config.update_steps, desc="Training"):
        try:
            ...
            actions = actions.reshape(-1, config.seq_len, config.action_dim)
            ...
            print("Predicted: ", predicted_actions, "Real: ", actions)
